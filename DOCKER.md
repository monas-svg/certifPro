...
- Le healthcheck Docker interroge `/healthz` toutes les 30 secondes.
- Pour des volumes de trafic importants, ajoutez une vraie base de données
  (Postgres, par ex.) à la place de la `Map` en mémoire utilisée dans
  `server.js` pour stocker les commandes — actuellement, un redémarrage du
  conteneur efface les commandes en cours.
